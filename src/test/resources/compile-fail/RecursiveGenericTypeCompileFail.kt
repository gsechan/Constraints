...
fun test() {
    ...
    val a: List<@CollectionSize(1, 10) List<@IntRange(2, 5) Int>> = listOf(listOf(20)) // ERROR: can never be valid

    // Inner collection provably the wrong size (0 not in [1, 10], depth 1) -> hard error.
    val b: List<@CollectionSize(1, 10) List<@IntRange(2, 5) Int>> = listOf(emptyList()) // ERROR: can never be valid

    // Dynamic nested source, not a known transfer -> needs checkConstraint.
    val source = listOf(listOf(3)).filter { it.isNotEmpty() }
    val c: List<@CollectionSize(1, 10) List<@IntRange(2, 5) Int>> = source // ERROR: Cannot prove every element satisfies

    // Transfer from a value WITHOUT the nested element-type constraints is not proven.
    val plain = listOf(listOf(3))
    val d: List<@CollectionSize(1, 10) List<@IntRange(2, 5) Int>> = plain // ERROR: Cannot prove every element satisfies
}
