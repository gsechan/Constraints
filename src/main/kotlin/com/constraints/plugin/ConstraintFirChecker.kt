...
object ConstraintReturnChecker : FirReturnExpressionChecker(MppCheckerKind.Common) {
    ...
    override fun check(expression: FirReturnExpression) {
        ...
        if (verifyElementTypeConstraints(function.symbol.resolvedReturnType.elementTypeConstraintKeys(context.session), result, context, reporter)) return
...
private fun verifyConstraints(
    ...
    val requiredElementType = symbol.resolvedReturnType.elementTypeConstraintKeys(context.session)
    ...
    if (required.isEmpty() && requiredElement.isEmpty() &&  requiredElementType.isEmpty() && range == null && 
        doubleRange == null && stringLength == null && collectionSize == null && stringMatches.isEmpty() && divisibility == null) return
    ...
    if (verifyElementTypeConstraints(requiredElementType, rhs, context, reporter)) return
