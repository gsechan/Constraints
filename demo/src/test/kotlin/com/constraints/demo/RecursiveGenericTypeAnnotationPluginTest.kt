...
/**
 * Element-type constraints nest: `List<@Size(1, 10) List<@IntRange(2, 5) Int>>` means every
...
class RecursiveGenericTypeAnnotationPluginTest {
    ...
    fun `builder of provably-valid nested literals compiles without checkConstraint`() {
        ...
        val xs: List<@Size(1, 10) List<@IntRange(2, 5) Int>> = listOf(listOf(3))
        assertEquals(3, xs[0][0])
    }

    @Test
    fun `builder with several valid inner lists compiles`() {
        val xs: List<@Size(1, 10) List<@IntRange(2, 5) Int>> = listOf(listOf(2, 5), listOf(3, 4))
        assertEquals(2, xs.size)
    }

    @Test
    fun `checkConstraint passes for valid nested collections`() {
        val xs: List<@Size(1, 10) List<@IntRange(2, 5) Int>> = checkConstraint(listOf(listOf(2, 3), listOf(4)))
        assertEquals(2, xs.size)
    }
    ...
    fun `checkConstraint throws when an inner element is out of range (depth 2)`() {
        assertFailsWith<ConstraintException> {
            val xs: List<@Size(1, 10) List<@IntRange(2, 5) Int>> =
    ...
    fun `transfer between values with the same nested element-type constraints compiles`() {
        val a: List<@Size(1, 10) List<@IntRange(2, 5) Int>> = checkConstraint(listOf(listOf(3)))
        val b: List<@Size(1, 10) List<@IntRange(2, 5) Int>> = a // proven: identical nested constraints
        assertEquals(a, b)
    }
